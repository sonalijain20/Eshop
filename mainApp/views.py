...
def shopDetails(request,cat,br):
    ...
    if(cat=='default' and br=='default'):
        ...
        cobj = Category.objects.get(name=cat)
        bobj = Brand.objects.get(name=br)
        p=Product.objects.filter(category=cobj, brand=bobj)
    ...
    if(request.method=='POST'):
        try:
            ...
            if (request.POST.get('black') == '1'):
                p.black = False
            if (request.POST.get('mustard') == '2'):
                p.mustard = True
            if (request.POST.get('mustard') == '1'):
                p.mustard = False
            if (request.POST.get('blue') == '2'):
                p.blue = True
            if (request.POST.get('blue') == '1'):
                p.blue = False
...
def editProduct(request, num):
    ...
    if(request.method=='POST'):
        ...
        p.save()
        return HttpResponseRedirect('/profile/')
    return render(request, "editproduct.html", {"Product":p, "Brand":brand, "Category":category})


@login_required(login_url='/login/')
def deleteCart(requesr, num):
